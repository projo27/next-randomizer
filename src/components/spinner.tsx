...
import { Wand2, Trash2, Copy, Check, Star } from "lucide-react";
    ...
    setTimeout(() => {
      ...
      setIsSpinning(false);
       toast({
        ...
        description: <p className="text-xl font-bold">{newWinner}</p>,
      });
    }, 5000); // Must match animation duration in globals.css
  };
      ...
      <CardContent className="grid md:grid-cols-2 gap-8 items-center">
        <div className="relative">
          <div className="aspect-square p-4 relative flex items-center justify-center">
              <div className="absolute w-full h-full spinner-arrow">
                <Star className="w-12 h-12 text-accent fill-current -translate-y-2" />
              </div>
              <div 
              ...
              >
                  {items.map((item, index) => {
                      ...
                      return (
                           ...
                           >
                            <span 
                                className="transform -rotate-90 text-center px-2 truncate block max-w-full"
                                style={{ transform: `rotate(${segmentAngle > 90 ? 0 : -90}deg) translate(-50%, -50%)` }}
                            >
                                {item}
                            </span>
                           </div>

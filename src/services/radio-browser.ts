...
const RadioStationSchema = z.object({
  ...
  languagecodes: z.string().optional().or(z.literal("")),
  votes: z.number(),
...
async function fetchFromApi<T>(
  ...
  try {
    ...
    if (!validation.success) {
      console.error(
        "Radio Browser API Zod validation error:",
        validation.error.issues,
        validation.error.errors,
      );
      throw new Error(
        "Received unexpected data format from Radio Browser API.",
      );
    }
    ...
    throw new Error(
      "An unknown error occurred while fetching from Radio Browser API.",
    );
  }
}

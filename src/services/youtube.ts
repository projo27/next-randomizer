  ...
  try {
    ...
    if (apiError.errors && apiError.errors.length > 0) {
      ...
      if (
        ...
        throw new Error(
          `The YouTube API key is invalid or misconfigured. Reason: ${reason}`,
        );
      }
      if (reason === "processingFailure" || apiError.code === 400) {
        console.warn(
          `Could not fetch videos for category ${videoCategoryId} in region ${regionCode}.`,
        );
        return []; // Return empty array to allow frontend to handle it gracefully
      }
    }
    throw new Error(
      "An unexpected error occurred while fetching videos from YouTube.",
    );
  }
}
